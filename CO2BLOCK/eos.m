...
function [brineviscosity, co2density, co2viscosity] = eos(T,p,salinity,co2dens)   % T in ºC, p in MPa, salinity in [ppm/1e6]
    ...
    co2density = calculate_co2_density(T, p);
...
function co2density = calculate_co2_density(T, p)
    % 定义常数
    a0 = 7.54;  % 常数 [Pa m^6 K^0.5 mol^-2]
    a1 = -4.13*10^-3;  % 常数 [Pa m^6 K^0.5 mol^-2]
    b = 2.78*10^-5;  % 常数 [m^3/mol]
    R = 8.314472;  % 气体常数 [m^3 Pa K^-1 mol^-1]
    
    % 计算参数 a
    a = a0 + a1 * T;

    % 定义方程
    eqn = @(x) x.^3 - (R * T / p) * x.^2 - (R * T * b / p - a / p / sqrt(T) + b.^2) * x - (a * b / p / sqrt(T));

    % 设置初始猜测值
    initial_guess = 1e-4;  % 适当的初始猜测值，根据问题需要调整

    % 使用 fsolve 进行数值求解
    options = optimset('Display', 'off');  % 关闭显示
    V = fsolve(eqn, initial_guess, options);  % 求解方程
    
    % 计算 CO2 密度
    co2density = 0.044 / V;  % [kg/m^3]
end
