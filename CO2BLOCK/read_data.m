...
function [thick,area_res,perm,por,dens_c,visc_c,visc_w,compr,p_lim,rc,...
    ...
    disp(['Domain Type: ', domain_type]);
    disp(['Depth: ', num2str(depth), ' m']);
    disp(['Mean Depth: ', num2str(depth_mean), ' m']);
    disp(['Thickness: ', num2str(thick), ' m']);
    disp(['Area of Reservoir: ', num2str(area_res), ' km^2']);
    disp(['Intrinsic Permeability: ', num2str(perm), ' m^2']);
    disp(['Porosity: ', num2str(por)]);
    disp(['Rock Compressibility: ', num2str(cr), ' 1/Pa']);
    disp(['Water Compressibility: ', num2str(cw), ' 1/Pa']);
    disp(['Density of CO2: ', num2str(dens_c), ' kg/m^3']);
    disp(['Viscosity of CO2: ', num2str(visc_c), ' Pa.s']);
    disp(['Viscosity of Water: ', num2str(visc_w), ' Pa.s']);
    disp(['Pressure at Top of Reservoir: ', num2str(pres0), ' MPa']);
    disp(['Pressure at Centre of Reservoir: ', num2str(pres0_mean), ' MPa']);
    disp(['Temperature at Centre of Reservoir: ', num2str(T0_mean), ' °C']);
    disp(['Aquifer Salinity: ', num2str(salinity), ' ppm']);
    disp(['Total Maximum Principal Stress: ', num2str(s1_tot), ' MPa']);
    disp(['Stress Ratio: ', num2str(stress_ratio)]);
    disp(['Rock Friction Angle: ', num2str(friction), ' deg']);
    disp(['Rock Cohesion Coefficient: ', num2str(cohesion), ' MPa']);
    disp(['Rock Tensile Strength: ', num2str(tens_strength), ' MPa']);
